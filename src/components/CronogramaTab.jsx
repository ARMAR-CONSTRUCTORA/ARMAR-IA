...
function StatsPanel({ tareas, avanceGeneral, informes }) {
  ...
  const presupuestoTotal = tareas.filter(t => t.parentId === null && t.presupuesto).reduce((s, t) => s + t.presupuesto, 0)
  ...
  return (
    <div style={{ width: 210, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {presupuestoTotal > 0 && (
        <div style={{ background: 'white', border: '1px solid var(--gray-200)', borderRadius: 12, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', textAlign: 'center' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Presupuesto total</div>
          <div style={{ fontSize: 18, fontWeight: 900, color: 'var(--orange)' }}>
            {fmtPesos(presupuestoTotal)}
          </div>
        </div>
      )}
...
function TablaGantt({ tareas, structuralMode, onClickTarea, onDeleteTarea, onAddSubtarea, ppd, onZoomChange }) {
  ...
  const COL_PRESUPUESTO = 110
  const TABLE_W = COL_NOMBRE + COL_INICIO + COL_FIN + COL_DIAS + COL_AVANCE + COL_ESTADO + COL_CRITICA + COL_PRESUPUESTO
  ...
  const renderRow = (tarea, isSubtarea = false) => {
    ...
    return (
      <div key={tarea.id} style={{ display: 'flex', height: ROW_H, borderBottom: '1px solid var(--gray-200)' }}>
        ...
        <div style={{ ...cellStyle(COL_PRESUPUESTO), display: 'flex', alignItems: 'center', padding: '0 8px', background: isSubtarea ? 'white' : '#FAFAFA' }}>
          <span style={{ fontSize: 10, color: tarea.presupuesto ? 'var(--gray-700)' : 'var(--gray-300)', fontWeight: tarea.presupuesto ? 600 : 400 }}>
            {fmtPesos(tarea.presupuesto)}
          </span>
        </div>
      ...
      <div data-gantt-content style={{ minWidth: TABLE_W + timelineW + (structuralMode ? 32 : 0) }}>
        <div style={{ display: 'flex', height: HEADER_H, background: '#F3F4F6', borderBottom: '2px solid var(--gray-200)', position: 'sticky', top: 0, zIndex: 10 }}>
          {[
            ...
            { label: 'Presupuesto',   w: COL_PRESUPUESTO },
          ].map(col => (
            ...
            return (
              <svg style={{ position: 'absolute', top: 0, left: TABLE_W, pointerEvents: 'none', overflow: 'visible', zIndex: 5 }} width={timelineW} height={svgH}>
                {arrows.map(dep => {
                  ...
                  const x1 = toPx(pred.fechaFin) + ppd
                  const x2 = toPx(dep.fechaInicio)
