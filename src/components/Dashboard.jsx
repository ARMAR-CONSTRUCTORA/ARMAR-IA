...
function KPICard({ label, value, icon, valueColor, bg, border }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 26, fontWeight: 900, color: valueColor, lineHeight: 1, letterSpacing: '-1px' }}>
          {value}
        </div>
        <div style={{ color: 'var(--gray-500)', fontSize: 12, fontWeight: 500, marginTop: 4 }}>
          {label}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{
            fontSize: isMobile ? 24 : 28, fontWeight: 900, color: 'var(--gray-900)',
      ...
      <div style={{
        ...
        <KPICard label="Terminadas"       value={terminadas} icon="✅" valueColor="#2563EB"         bg="#DBEAFE"          border="#93C5FD" />
        <KPICard label="Atrasadas"        value={atrasadas}  icon="⚠️" valueColor="#DC2626"         bg="#FEE2E2"          border="#FCA5A5" />
      </div>

      {/* Two-column layout — stacks on mobile/tablet */}
      <div style={{
        display: 'grid',
        gridTemplateColumns: compact ? '1fr' : '1fr 360px',
        gap: isMobile ? 16 : 20,
        alignItems: 'start',
      }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--gray-800)' }}>Obras Activas</h2>
              <p style={{ fontSize: 12, color: 'var(--gray-400)', marginTop: 2 }}>Avance en ejecución</p>
            </div>
            <button
              onClick={onNavigateToObras}
              style={{
                background: 'none', border: '1px solid var(--gray-200)', borderRadius: 7,
                padding: '6px 14px', color: 'var(--gray-600)', fontSize: 12,
            ...
            <div key={p.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                ...
                <span style={{
                  ...
                  color: STATUS[p.status].color, background: STATUS[p.status].bg, flexShrink: 0,
                }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--gray-400)', marginTop: 5 }}>
                Término est.: {formatDate(p.endDate)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 52, fontWeight: 900, letterSpacing: '-2px', lineHeight: 1 }}>
              {avgProgress}%
            </div>
            <div style={{
              marginTop: 18, height: 8, background: 'rgba(255,255,255,0.25)',
              borderRadius: 99, overflow: 'hidden',
            }}>
              <div style={{
                width: `${avgProgress}%`, height: '100%',
                background: 'white', borderRadius: 99, transition: 'width 0.4s',
              }} />
            </div>
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--gray-800)', marginBottom: 16 }}>
              ...
              <div key={label} style={{ marginBottom: 14 }}>
                ...
                <div style={{ height: 6, background: '#F3F4F6', borderRadius: 99, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    height: '100%', background: color, borderRadius: 99, transition: 'width 0.4s',
                  }} />
                </div>
              </div>
            ))}
          ...
          {atrasadasList.length > 0 && (
            <div style={{
              background: 'white', borderRadius: 14,
              boxShadow: '0 1px 6px rgba(0,0,0,0.07)',
              border: '1px solid #FECACA', padding: '18px 20px',
            }}>
              ...
              {atrasadasList.map((p, i) => (
                <div key={p.id} style={{
                  ...
                  <div style={{ fontWeight: 600, color: 'var(--gray-700)', fontSize: 13 }}>{p.name}</div>
